...
#from common import *
from common import NODE_TYPE, VERTEX_TYPE, NETWORK_TYPE
from common import INTERFACE_TYPE, INTERNAL_EDGE, LINK_EDGE, PEER_TYPE
from common import datatype_route_cost, datatype_route_bgp
from common import z3_interface_links
...
def fill_box_info(box_name, unrolling_limit):
    """
    Initialize the dict for each synthesis box
    :param box_name: string the name, must be in BOXES 
    :param unrolling_limit: 
    :return: None
    """
    assert box_name in BOXES, "Undefined Box name"
    ...
    # Fill and input and outputs for each box
...
class Synthesizer(object):
    ...
    def _fill_boxes(self):
        ...
        for box_name in self.boxes_names:
            ...
            for i, c in enumerate(BOXES[box_name]['input_constraints']):
                if c is None: continue
                #print "Added constrains", box_name, c
                if not isinstance(c, list):
                    name = '%s_inc%d' % (box_name, i)
                    BOXES[box_name]['solver'].assert_and_track(c, name)
                else:
                    for j, cj in enumerate(c):
                        name = '%s_inc%d_%d' % (box_name, i, j)
                        BOXES[box_name]['solver'].assert_and_track(cj, name)
    ...
    def synthesize(self):
        ...
        while True:
            ...
            for func_name, func in BOXES[box_name]['outputs'].iteritems():
                ...
                return_else = True
                if orig_name in ['BGPLocalPref', 'OSPFRoute', 'BGPRoute', 'BGPAnnouncement']:
                    return_else = False
            ...
            for func_name, func in BOXES[box_name]['inputs'].iteritems():
                orig_name = get_original_version(func_name)
                return_else = True
                if orig_name in ['BGPLocalPref', 'OSPFRoute', 'BGPRoute', 'nonMinOSPFRouteCost', 'BGPAnnouncement', 'nonMaxBGPLocalPref']:
                    return_else = False
                if orig_name in yes_func_vals:
                    fed_output = self._process_vals(
                        func,
            ...
            if solver.check() == z3.sat:
                ...
                print "Double chck", solver.check()
    ...
    def generate_function_constraints(self, func):
        ...
        for i, t in enumerate(FUNCS_SIG[func_name]):
            if t == 'Vertex':
                ...
                tmp = z3.Const('t%d' % i, self.vertex)
                variables.append(tmp)
                if func_name == 'BGPRoute': continue
    ...
    def fill_boxes_input_constraints(self, box_name):
        ...
        # Propagate all BGP routes
        if BGPRoute is not None:
            c = z3.ForAll(
                (v1 , v2, v3, s1, t1, t2),
                z3.Implies(
                  z3.Or(
                      [
                          z3.And(
                              ann[0] != v1, ann[1] == v2, v3 == ann[2], ann[3] == s1, ann[4] == t1, t2 == 6) for ann in self.bgp_annoucements
                      ]),
                    BGPRoute(v1, v2, v3, s1, t1, t2)
                ))
            constraints.append(c)
    ...
    def _get_names(self):
        ...
        network_names = list(set(network_names) - set(announced_network_names))
        self.as_paths = list(set(as_paths))
        self.announced_network_names = list(set(announced_network_names) - set(network_names))
        ...
        all = node_names + interface_names + network_names + self.announced_network_names
        assert len(all) == len(set(all))
        return node_names, interface_names, network_names, self.announced_network_names
